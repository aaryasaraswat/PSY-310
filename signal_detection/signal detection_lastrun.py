...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/aaryasaraswat/Desktop/LAB_PSY/signal_detection/signal detection_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            # Run 'Each Frame' code from keypress
            # Define the correct key for this trial (e.g., from your conditions file)
            correct_key = 'up'  # Change this depending on the correct answer for the trial
            
            # Get the key pressed by the participant
            keys = event.getKeys(keyList=['up', 'down'])
            
            if 'up' in keys:
                thisExp.addData('keyPressed', 'up')
                if 'up' == correct_key:  # Compare with the correct key
                    thisExp.addData('correct', 1)  # 1 means correct
                else:
                    thisExp.addData('correct', 0)  # 0 means incorrect
                continueRoutine = False  # End the routine if you want to move to the next trial
            
            if 'down' in keys:
                thisExp.addData('keyPressed', 'down')
                if 'down' == correct_key:  # Compare with the correct key
                    thisExp.addData('correct', 1)  # 1 means correct
                else:
                    thisExp.addData('correct', 0)  # 0 means incorrect
                continueRoutine = False  # End the routine if you want to move to the next trial
